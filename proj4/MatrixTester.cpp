...
void MatrixTester::testCopyConstructor() {
	...
	// copy empty Matrix<int>
	Matrix<int> m1;
	Matrix<int> m2 = Matrix<int>(m1);
	assert( m2.getRows() == 0 );
	assert( m2.getColumns() == 0 );
	assert( m2.myVec.getSize() == 0 );
	cout << " 1 " << flush;

	// copy non-empty Matrix<int>
	Matrix<int> m3(2, 3);
...
void MatrixTester::testSubscripts() {
	...
	try {
		double value = m0[0][0];
		cerr << "successfully read from empty Matrix<int>";
		exit(1);
	} catch (range_error&) {
		cout << " 3 " << flush;
 	}
	...
	try {
		m0[0][0] = 0;
		cerr << "successfully wrote to empty Matrix<int>";
		exit(1);
	} catch (range_error&) {
		cout << " 4 " << flush;
 	}
...
void MatrixTester::testWriteToStream() {
	...
	m.writeTo(fout);
	fout.close();
	// now, read what we just wrote into a different Matrix<int>
	Matrix<int> m1;
